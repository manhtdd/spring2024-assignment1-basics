import pickle
from typing import Literal, Iterable, Iterator, List, Dict, Tuple
...
DATASET_PATHS = {'owt': '/media/aiotlab3/27934be5-a11a-44ba-8b28-750d135bc3b3/manhtd/stanford-cs336/data/owt_train.txt',
                 'tinystories': '/media/aiotlab3/27934be5-a11a-44ba-8b28-750d135bc3b3/manhtd/stanford-cs336/data/TinyStoriesV2-GPT4-train.txt'}
DATASET_VALID_PATHS = {'owt': '/media/aiotlab3/27934be5-a11a-44ba-8b28-750d135bc3b3/manhtd/stanford-cs336/data/owt_valid.txt',
                       'tinystories': '/media/aiotlab3/27934be5-a11a-44ba-8b28-750d135bc3b3/manhtd/stanford-cs336/data/TinyStoriesV2-GPT4-valid.txt'}
...
def train_bpe(in_string: bytes, vocab_size: int, special_tokens: List[str]) -> Tuple[Dict[str, bytes], List[Tuple[bytes, bytes]]]:
    assert vocab_size <= 2**16, "vocab_size must be less than 2^16"
...
class BPETokenizer:
    ...
    def __init__(self, vocab: dict[int, bytes], merges: list[tuple[bytes, bytes]], special_tokens: list[str] | None = None):
        ...
        if special_tokens is None:
            special_tokens = []
        self.tokenizer = Tokenizer(vocab, merges, special_tokens)
...
def sample_and_compress(dataset: Literal['owt', 'tinystories'], with_tokenizer_from: Literal['owt', 'tinystories'] | None = None):
    ...
    first_10_lines = next(
        iter(re.finditer(r"([\s\S]*?<\|endoftext\|>){10}", text))).group(0)
...
def tokenize_full_dataset(dataset: Literal['owt', 'tinystories']):
    ...
    with open(f'/media/aiotlab3/27934be5-a11a-44ba-8b28-750d135bc3b3/manhtd/stanford-cs336/data/{dataset}_train_tokens.npy', 'wb') as f:
        np.save(f, tokens)

    with open(DATASET_VALID_PATHS[dataset], 'rb') as f:
        text = f.read()
    tokens = tokenizer.encode(text, as_list=False)
    with open(f'/media/aiotlab3/27934be5-a11a-44ba-8b28-750d135bc3b3/manhtd/stanford-cs336/data/{dataset}_valid_tokens.npy', 'wb') as f:
        np.save(f, tokens)


def test_training():
    with open('/media/aiotlab3/27934be5-a11a-44ba-8b28-750d135bc3b3/manhtd/stanford-cs336/data/bpe_trivial.txt', 'r') as f:
        text = f.read()
