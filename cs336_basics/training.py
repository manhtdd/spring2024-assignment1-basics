...
def load_data(dataset: npt.NDArray, batch_size: int, context_length: int, device: str) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    samples = np.stack([dataset[idx:idx + context_length]
                       for idx in sample_idcs])
    samples_offset = np.stack(
        [dataset[idx + 1:idx + context_length + 1] for idx in sample_idcs])
...
def parse_args():
    ...
    parser.add_argument('--dataset', default='owt',
                        type=str, choices=['owt', 'tinystories'])

    parser.add_argument('--d_ff', default=2048, type=int)
    parser.add_argument('--total_steps', default=70_000,
                        type=int)  # Just an estimate
...
def train_model():
    ...
    model = Transformer(tie_embeddings=args.tie_embeddings, vocab_size=vocab_size, activation=args.activation, context_length=args.context_length, num_layers=args.num_layer,
                        d_model=args.d_model, num_heads=args.num_heads, d_ff=args.d_ff, parallel_layers=args.parallel_layers, post_norm=args.post_norm, device='cuda')
    ...
    train_dataset = np.memmap(
        f'/media/aiotlab3/27934be5-a11a-44ba-8b28-750d135bc3b3/manhtd/stanford-cs336/data/{args.dataset}_train_tokens.npy', dtype=np.uint16, mode='r')
    valid_dataset = np.memmap(
        f'/media/aiotlab3/27934be5-a11a-44ba-8b28-750d135bc3b3/manhtd/stanford-cs336/data/{args.dataset}_valid_tokens.npy', dtype=np.uint16, mode='r')
    ...
    lr_schedule = partial(cosine_with_warmup_lr_schedule, max_learning_rate=args.lr, min_learning_rate=args.lr *
                          args.decay, warmup_iters=warmup_iters, cosine_cycle_iters=schedule_end_time)

    with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
        model.train()
        section_training_loss = (0.0, 0)
        try:
            pbar = tqdm(data_generator(train_dataset, batch_size=args.batch_size,
                        context_length=args.context_length, device='cuda'))
            for training_step, (train_x, train_y) in enumerate(pbar):
                # Maybe fix crashes that happen very seldomly
                train_x = torch.minimum(train_x, torch.tensor(
                    vocab_size - 1, device='cuda'))
                train_y = torch.minimum(train_y, torch.tensor(
                    vocab_size - 1, device='cuda'))
                ...
                section_training_loss = (
                    section_training_loss[0] + training_loss.item(), section_training_loss[1] + 1)
                del training_loss
                if training_step % args.checkpoint_interval == 0 and training_step != 0:
                    save_checkpoint(model, optimizer, training_step, checkpoints_dir / f'{
                                    wandb.run.name}_{time_str}_{training_step // 1000}k')
                if training_step % args.log_interval == 0:
                    model.eval()
                    # Compute validation loss
                    valid_x, valid_y = load_data(
                        valid_dataset, batch_size=args.batch_size, context_length=args.context_length, device='cuda')
                    with torch.no_grad():
                        valid_loss = loss_func(model(valid_x), valid_y)
                    training_loss_avg = section_training_loss[0] / \
                        section_training_loss[1]
                    section_training_loss = (0.0, 0)
                    if training_step == 0:
                        wandb.init(
                    ...
                    wandb.log({'loss/train': training_loss_avg},
                              step=training_step)
                    wandb.log(
                        {'perplexity/train': torch.exp(torch.tensor(training_loss_avg))}, step=training_step)
                    wandb.log({'loss/valid': valid_loss.item()},
                              step=training_step)
                    wandb.log(
                        {'perplexity/valid': torch.exp(valid_loss).item()}, step=training_step)
                    wandb.log(
                        {'lr': optimizer.param_groups[0]['lr']}, step=training_step)
                    pbar.set_postfix(
                        {'train_loss': training_loss_avg, 'valid_loss': valid_loss.item()})
                    model.train()
